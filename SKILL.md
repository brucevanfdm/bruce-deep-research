...
跳过正式计划，直接在心里明确"要搞清楚什么"和"怎么搜"，然后立即执行搜索。不写计划、不建 TodoWrite、不创建文件夹。搜完直接作答。
...
**计划落入 TodoWrite**：把每个核心问题 / 信息维度拆成独立任务写入 TodoWrite，后续搜索过程中逐项标记完成。这样不会遗漏维度，用户也能看到进度。

**等待用户确认**：展示计划后，询问用户是否同意方向、是否需要调整维度或补充关注点。确认后再开始搜索。这一步能避免跑偏整套流程后才发现方向不对。
...
**存储时机**：每成功抓取一个有价值的来源，立即写入一个素材文件。不要等搜完所有来源再批量写入——中途上下文可能被压缩，信息会丢失。
...
1. **第一轮——全局了解**：搜索主体，读官网 + 2–3 篇新闻/简介。每读完一个有价值的来源，立即存入 `sources/`。
2. **⏸ 中期检查点**：向用户汇报第一轮发现的概况，确认：
   - 方向是否正确？
   - 是否需要调整关注维度？
   - 有没有用户已知的信息可以补充？
3. **第二轮——补充深度**：针对第一轮的空白定向搜索。继续沉淀素材。
4. 重要结论需至少 2 个来源交叉验证。
5. 更新 `notes.md`，记录关键发现和矛盾点。

### 深度模式

1. **第一轮——全景摸底**：广泛搜索，识别所有主要玩家/维度/来源。沉淀素材。
2. **⏸ 方向确认点**：向用户汇报发现了哪些主要玩家和维度，确认是否全面、是否需要聚焦或扩展范围。
3. **第二轮——逐一深挖**：每个对象分别读官网+新闻+社区讨论。持续沉淀。
4. **第三轮——信号挖掘**：招聘页面（透露技术方向和增长计划）、GitHub 仓库、产品更新日志、融资新闻、用户评价（G2/Capterra/AppStore/知乎/小红书）。
5. **⏸ 深度检查点**：向用户汇报阶段性成果，确认是否继续深挖或可以进入总结。
6. **第四轮——填补空白**：回看研究计划（TodoWrite），还有什么未完成？针对性补搜。
7. **第五轮+（必要时）**：交叉验证、矛盾信息核查、时效性确认。
8. 更新 `notes.md`。
